$(function() {
  ...
  function buscar_listado() {
    ...
    setTimeout(() => {
      $.ajax({
        url: url + "controllers/c_modulo_oficio.php",
        type: "POST",
        dataType: "JSON",
        data: {
          opcion: "Consultar",
        ...
        success: function(resultados) {
          ...
          if (dataListado.resultados) {
            for (var i in dataListado.resultados) {
              ...
              contenido +=
                '<td class="py-2 px-1">' +
                dataListado.resultados[i].codigoModuloOficio +
                "</td>";
              contenido +=
                '<td class="py-2 px-1">' +
              ...
              contenido +=
                ...
                "</td>";
              contenido +=
                '<td class="text-center py-2 px-1">' + estatus_td + "</td>";
              ////////////////////////////////////////////////////////
              if (permisos.modificar == 1 || permisos.act_desc == 1) {
        ...
        error: function(msjError) {
          ...
          console.log(
            "Error: " + msjError.status + " - " + msjError.statusText
          );
  ...
  function verificarParte1() {
    ...
    let codigo = $("#codigoModuloOficio").val();
    if (codigo != "") {
      if (codigo.match(ER_codigoFormulario)) {
        $("#codigoModuloOficio").css("background-color", colorb);
      } else {
        $("#codigoModuloOficio").css("background-color", colorm);
        pestania1 = false;
      }
    } else {
      $("#codigoModuloOficio").css("background-color", colorm);
  ...
  function editarRegistro() {
    ...
    $("#codigoModuloOficio").val(
      dataListado.resultados[posicion].codigoModuloOficio
    );
        ...
        $.ajax({
          ...
          success: function(resultados) {
            ...
            if (
              ...
              resultados == "Modificación exitosa"
            ) {
              ...
              resultados == "Registro fallido" ||
              resultados == "Modificación fallida"
            ) {
  ...
  function cambiarEstatus(e) {
    ...
    setTimeout(() => {
      $.ajax({
        ...
        data: {
          opcion: "Estatus",
          codigoModuloOficio: codigoModuloOficio,
          estatus: estatus,
        },
        success: function(resultados) {
          let color_alerta = "";
      ...
      success: function(resultados) {
        try {
          ...
          if (data.oficios) {
            ...
            dataoficios = data.oficios;
          } else {
            $("#oficio").html('<option value="">No hay módulos</option>');
          }
