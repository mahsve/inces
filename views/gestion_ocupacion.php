<?php
    ...
    {
        include_once('models/m_ocupacion.php'); // INCLUIMOS EL MODELO PARA CONSULTAR A LA BASE DE DATOS.
        $objeto = new model_ocupacion(); // CREAMOS UN NUEVO OBJETO PARA ACCEDER A LOS METODOS.

        $arreglo = ['codigo' => htmlspecialchars($dataGET[1])]; // GUARDAMOS LOS DATOS EN UN ARREGLO.

        $objeto->conectar(); // CONECTAMOS A LA BASE DE DATOS.
        $consulta = $objeto->consultarOcupacion($arreglo); // CONSULTAMOS EL ROL CON SU CODIGO.
...
<div class="d-flex align-items-center justify-content-between border-bottom pb-2 mb-2">
    ...
    <a href="<?php echo SERVERURL.'intranet/ocupacion'; ?>" class="btn btn-sm btn-info"><i class="fas fa-reply"></i><span class="ml-1">volver</span></a>
</div>

<form class="formulario" action="<?php echo SERVERURL.'controllers/c_ocupacion.php'; ?>" method="POST" name="formulario">
