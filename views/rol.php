<?php
include_once('models/m_rol.php');
$objeto = new model_rol();
$objeto->conectar();
$roles = $objeto->consultarRoles();
...
<div class="d-flex align-items-center justify-content-between border-bottom pb-2 mb-2">
    ...
    <?php if ($permisos['registrar']) { ?>
        <a href="gestion_rol" class="btn btn-sm btn-info"><i class="fas fa-plus"></i><span class="ml-1">Registrar</span></a>
    <?php } ?>
</div>
        ...
        <tr class="text-capitalize">
            ...
            <td class="align-middle p-2">
                <?php if ($permisos['modificar']) { ?>
                    <a href="<?php echo 'gestion_rol/'.$datos['codigo']; ?>" class="btn btn-sm btn-info"><i class="fas fa-edit"></i></a>
                <?php } ?>

                <?php
                if ($permisos['eliminar']) {
                    if ($_SESSION['usuario']['codigo_rol'] != $datos['codigo']) { ?>
