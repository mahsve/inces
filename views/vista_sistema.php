<?php
include_once('models/m_vistas.php');
$objeto = new modelo_vistas();
$objeto->conectar();
$vistas = $objeto->consultarVistas();
...
<div class="d-flex align-items-center justify-content-between border-bottom pb-2 mb-2">
    ...
    <?php if ($permisos['registrar']) { ?>
        <a href="gestion_vista_sistema" class="btn btn-sm btn-info"><i class="fas fa-plus"></i><span class="ml-1">Registrar</span></a>
    <?php } ?>
</div>
...
<table id="listado" class="table table-bordered table-hover w-100">
    <thead class="">
        <tr class="text-capitalize">
            ...
            <th class="font-weight-normal p-2">Modulo</th>
    ...
    <tbody>
        <?php
        if ($vistas) {
            foreach ($vistas AS $datos) {
        ?>
        <tr class="text-capitalize">
            <td class="align-middle text-right"><?php echo $datos['codigo']; ?></td>
            <td class="align-middle"><?php echo $datos['nombre']; ?></td>
            <td class="align-middle"><?php echo $datos['modulo']; ?></td>

            <?php if ($permisos['modificar'] OR $permisos['act_desc'] OR $permisos['eliminar']) { ?>
            <td class="align-middle p-2">
                <?php if ($permisos['modificar']) { ?>
                    <a href="<?php echo 'gestion_vista_sistema/'.$datos['codigo']; ?>" class="btn btn-sm btn-info"><i class="fas fa-edit"></i></a>
                <?php } ?>
...
<script>
    $(function () {
        ...
        function eliminar ()
        {
            ...
            $.ajax({
                url : '<?php echo SERVERURL.'controllers/c_vista.php'; ?>',
                type: 'POST',
                data: {
                    opcion: 'Eliminar',
                    codigo: codigo
